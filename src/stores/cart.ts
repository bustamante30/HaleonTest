  ...
  actions: {
    ...
    decorateCartOrders() {
      for (let i = 0; i < this.cartOrders.length; i++) {
        ...
        ReorderService.getThumbnail(this.cartOrders[i].originalOrderId).then(
          (response: string | boolean) => {
            if (response) this.cartOrders[i].thumbNailPath = response;
          },
        );
        ReorderService.decorateColours(this.cartOrders[i].colors);
        this.cartOrders[i].flattenedColors = this.flattenedColorsArrayDecorator(
          this.cartOrders[i].colors,
        );
      }
    },
