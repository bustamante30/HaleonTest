...
function removePlate(plateId: any) {
  const { colourId, data } = props
  const plate = data.find((plate: { id: any; }) => plate.id === plateId)
...
<template lang="pug">
.plates
  data-table.plate-details.p-datatable-sm(:value="data")
    column(v-for="(col, i) in config.cols" :field="col.field" :header="col.header" :headerStyle="stylify(col.width)" :bodyStyle="stylify(col.width)" :frozen="col.freeze ? true : false" :alignFrozen="col.freeze")
      template(#body="{ data }")
        table-cell(:config="col" :data="data" @update="updatePlate" :options="options")
    column
      template(#body="{ data }")
        sgs-button.sm.alert.secondary(@click="removePlate(data.id)" icon="delete")
  .actions
